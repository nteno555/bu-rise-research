...
from dataset import get_canonical_test_keys, get_dataset_splits, get_dataloaders
...
def evaluate_model(args):
    ...
    # Use the non-augmented dataset to define which images are in the test
    # set. This is the same split used during training, and is identical
    # for both models — eliminating sampling bias from independent re-splits.
    # ------------------------------------------------------------------
    canonical_test_dir = args.canonical_test_dir or args.dataset_dir
    print(f"Deriving canonical test split from: {canonical_test_dir}")
    fixed_test_keys = get_canonical_test_keys(canonical_test_dir, seed=args.seed)
    print(f"Canonical test set size: {len(fixed_test_keys)} images")

    print(f"Loading evaluation dataset from: {args.dataset_dir}")
    _, _, test_ds, eval_classes = get_dataset_splits(
        args.dataset_dir,
        input_size=input_size,
        seed=args.seed,
        subset_fraction=args.subset_fraction,
        fixed_test_keys=fixed_test_keys,
    )
    ...
    with open(report_path, "w") as f:
        ...
        f.write(f"Canonical test dir: {canonical_test_dir}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_dir", type=str, required=True,
                        help="Path to evaluation image dataset directory (non-augmented for fair comparison)")
    parser.add_argument("--canonical_test_dir", type=str, default=None,
                        help="Path to the non-augmented dataset used to define the canonical "
                             "test split. Defaults to --dataset_dir. Must be the same directory "
                             "used during training to ensure identical test images.")
    parser.add_argument("--model_name", type=str, required=True,
                        help="Name of model (e.g. non_augmented or augmented)")
    parser.add_argument("--eval_set_name", type=str, required=True,
                        help="Name of evaluation set (e.g. non_augmented_test)")
