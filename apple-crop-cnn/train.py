...
def train_model(args):
    ...
    # Derived from the non-augmented dataset so both models share the
    # exact same held-out test images regardless of training dataset size.
    # ------------------------------------------------------------------
    ...
    fixed_test_keys = get_canonical_test_keys(
        canonical_test_dir, seed=args.seed
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch_val_acc > best_val_acc:
            ...
            print(f"--> Saved new best model checkpoint to {checkpoint_path} "
                  f"with Val Acc: {epoch_val_acc:.4f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_dir", type=str, required=True,
                        help="Path to image dataset directory (augmented or non-augmented)")
    parser.add_argument("--dataset_name", type=str, required=True,
                        choices=["non_augmented", "augmented"],
    ...
    parser.add_argument("--canonical_test_dir", type=str, default=None,
                        help="Path to the non-augmented dataset used to define the canonical "
                             "test split. Both models must use the same value so they are "
                             "evaluated on identical images. Defaults to --dataset_dir if "
                             "not provided (fine when training the non-augmented model).")
    ...
    parser.add_argument("--save_dir", type=str, default="results",
                        help="Directory to save checkpoints/plots")
    parser.add_argument("--subset_fraction", type=float, default=1.0,
                        help="Fraction of dataset to use (for speed during development)")
